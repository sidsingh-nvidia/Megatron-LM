...
class KVBlockAllocator:
    ...
    def reconstruct_routing_from_blocks(
        ...
        Args:
            ...
            prefetched_blocks: Optional pre-snapshotted {block_id: routing}
                map, captured before update_requests() could release+reallocate
                any of these blocks to a different request. Checked first;
                falls back to self.block_routing (live lookup) per-block when a
                block id isn't present in the snapshot.
