...
class DynamicInferenceEngine(AbstractEngine):
    ...
    async def start_listening_to_data_parallel_coordinator(
        ...
        1.  **MP Coordinator (MP_rank=0, PP_rank=0)**: This rank connects directly
            ...
            to all other ranks within its model-parallel (TP) group.
        ...
        This architecture uses TCP sockets for intra-node and cross-node
        broadcasts within a TP/PP group. This enables cross-node Tensor Parallel (TP)
        and Pipeline Parallel (PP) groups.
        ...
        self.model_parallel_num_msgs_subscriber_socket.connect(
            f"tcp://{bcast_hostname}:{len_port}"
        )
    ...
    def schedule_requests(self) -> int:
        ...
        if tp_rank == 0 and pp_rank == 0:
            ...
            # are guaranteed to be available after the mp-rank 0 has sent them.
