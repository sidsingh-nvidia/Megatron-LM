...
def handle_submit_request(coordinator, sender_identity, metadata, bodies):
    ...
    print(f"[COORD][req={request_id}] pack meta: {(time.perf_counter()-_t_pack)*1000:.3f} ms  |  {len(engine_metadata)} bytes", flush=True)

    # Prefix-affinity routing is the only reason to look at the prompt. Under
    # load-balanced routing it stays an opaque frame for the whole crossing.
    _t_hash = time.perf_counter()
    if coordinator.routing_needs_prompt:
...
def handle_engine_reply(coordinator, sender_identity, metadata, bodies):
    ...
    ``metadata`` is ``[header, [[request_id, needs_detokenize], ...]]`` and
    ``bodies[i]`` is the packed finished request for entry ``i``. A body is
    decoded only when the coordinator has to detokenize into it; otherwise it is
    handed to the client as the same frame the engine produced.
    """
    ...
    for (fid, needs_detokenize), body in zip(metadata[1], bodies):
        ...
        print(f"[COORD][req={fid}] pack reply: {(time.perf_counter()-_t_pack_reply)*1000:.3f} ms  |  meta={len(reply_metadata)}B body={len(body)}B", flush=True)
